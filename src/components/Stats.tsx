...
export const Stats: React.FC = () => {
  ...
  const totalHours = Math.floor((stats.totalTimeSeconds || 0) / 3600);
  const totalMinutes = Math.floor(((stats.totalTimeSeconds || 0) % 3600) / 60);

  // Top tracks
  const tracksArr = Object.values(stats.tracksPlayed || {}) as any[];
  const topTracks = tracksArr.sort((a, b) => b.playCount - a.playCount).slice(0, 5);

  // Top artists
  const artistsArr = Object.entries(stats.artistsPlayed || {}).map(([name, count]) => ({ name, count: count as number }));
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl flex flex-col items-center justify-center text-center">
          ...
          <div className="text-3xl font-bold text-white mb-1">{totalHours}h {totalMinutes}m</div>
            ...
            {topTracks.length > 0 ? topTracks.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4">
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="text-white font-medium truncate">{item.track.name}</div>
                  <div className="text-sm text-white/50 truncate">{item.track.artist}</div>
                </div>
