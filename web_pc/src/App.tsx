...
const MainLayout: React.FC<MainLayoutProps> = ({
    ...
    const { tracks, globalSearchQuery } = useMusicContext();
    ...
    const renderView = (view: View) => {
        switch (view) {
            ...
            case 'upload':
                return (
                    <div className="space-y-5">
                        <div>
                            <h1 className="vl-view-title">Add Local Music</h1>
                            <p className="vl-view-subtitle">Select folders or files to add to your local library</p>
                        </div>
                        <FileUpload />
                    </div>
                );
            ...
            case 'search': {
                ...
                const filtered = query ? tracks.filter(t => 
                ...
                return (
                    <div className="space-y-5">
                        <div>
                            ...
                            <p className="vl-view-subtitle">
                                {query ? `Results for "${globalSearchQuery}"` : "Find your music"}
                            </p>
                        </div>
                        {query ? (
                            <>
                                <ArtistsGrid 
                                    ...
                                    searchQuery={query} 
                                    title="Artists" 
                                    onArtistClick={(artist) => { setOpenedArtist(artist); navigate('artistDetail'); }} 
                                />
                                <AlbumsGrid 
                                    mode="all" 
                                    searchQuery={query} 
                                ...
                                {filtered.length > 0 && (
                                    <div className="space-y-4">
                                        ...
                                        <TrackList tracks={filtered} showSearch={false} disableScroll={true} />
                                    </div>
                                )}
                                {filtered.length === 0 && (
                                    <div className="text-center text-white/50 py-10">
                                        No tracks found for "{globalSearchQuery}"
                                    </div>
                                )}
                            </>
                        ) : (
                            <div className="text-center text-white/50 py-10">
                                Type something in the search bar to find music.
                            </div>
                        )}
                    </div>
                );
            ...
            >
                ...
                <div className="vl-header" style={{ WebkitAppRegion: 'drag' as any }}>
                    ...
                    <div className="ml-8 z-50">
                        <GlobalSearch navigate={navigate} setOpenedAlbum={setOpenedAlbum} setOpenedArtist={setOpenedArtist} />
                    </div>
